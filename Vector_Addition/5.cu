...
__global__ void vector_add(float* __restrict__ a, float* __restrict__ b, float* c, int N) {
  ...
  // 让一个wrap来处理tail,  分担负载，加速优化
  int lane_id = threadIdx.x % 32;
  int tail_start = (N / 4) * 4;
  if (blockIdx.x == 0 && lane_id < (N - tail_start)) {
    int i = tail_start + lane_id;
    c[i] = a[i] + b[i];
  }
}
